...
public class Day14 extends PuzzleDay {
   ...
   * kitchen/laboratory.
   *
   * The Elves are trying to come up with the ultimate hot chocolate recipe; they're even 
   * maintaining a scoreboard which tracks the quality score (0-9) of each recipe.
   *
   ...
   * the score of their current recipe. So, after the first round, the first Elf moves forward 1 
   * + 3 = 4 times, while the second Elf moves forward 1 + 7 = 8 times. If they run out of 
   * recipes, they loop back around to the beginning. After the first round, both Elves happen to
   * loop around until they land on the same recipe that they had in the beginning; in general, 
   ...
   * recipes after the first nine on the scoreboard would be 5158916779 (highlighted in the last 
   * line of the diagram).
  ...
  public void solvePart1() {
    int afterNumRecipes = Integer.parseInt(getInput().get(0));
    Kitchen kitchen = new Kitchen();

    while(kitchen.recipes.size() < afterNumRecipes + 10){
      kitchen.cook();
    }
    
    for(int i = afterNumRecipes; i < afterNumRecipes + 10; i++){
      System.out.print(kitchen.recipes.get(i));
    }    
  ...
  public class Kitchen{
    ...
    public void cook(){
      int score = recipes.get(elfAIndex) + recipes.get(elfBIndex);
      recipes.addAll(splitDigits(score));
      elfAIndex = normalizeIndex(elfAIndex + recipes.get(elfAIndex).intValue() + 1);
      elfBIndex = normalizeIndex(elfBIndex + recipes.get(elfBIndex).intValue() + 1);
    ...
    public List<Integer> splitDigits(int numberInput){
  ...
  public void solvePart2() {
    List<Integer> search = getInput().get(0).chars()
    ...
    while(!found) {
      kitchen.cook();
      while (lastSearchIndex + search.size() <= kitchen.recipes.size() && 
              !(found = foundAtIndex(search, lastSearchIndex, kitchen.recipes))) {
        lastSearchIndex++;
      }
    }
    print(lastSearchIndex);
    return;
  }
  
  public boolean foundAtIndex(List<Integer> search, int indexStart, List<Integer> haystack){
    for(int i = 0; i < search.size(); i++){
      if(!haystack.get(indexStart + i).equals(search.get(i))){
        return false;
      }
    }
    return true;
