...
public class Day08 extends PuzzleDay {
   ...
   * numbers define a data structure which, when processed, produces some kind of tree that can
  ...
  public void solvePart1() {
    String inp = getInput().get(0);

    LinkedList<Integer> data = new ArrayList<>(Arrays.asList(inp.split(" "))).stream().map
            (Integer::parseInt)
            .collect
  ...
  public Node build(LinkedList<Integer> input){

    Node node = new Node();
    node.childCount = input.remove(0);
    node.metaCount = input.remove(0);
    if(node.childCount > 0){
     for(int i = 0; i < node.childCount; i++){
       node.children.add(build(input));
     }
    }
    for(int i = 0; i < node.metaCount; i++){
      node.meta.add(input.remove(0));
    }
  ...
  public class Node {
    ...
    public Integer sumMeta(){
      return meta.stream().mapToInt(Integer::intValue).sum() +
              children.stream().mapToInt(Node::sumMeta).sum();
    }

    public Integer weirdSum(){
      if(children.size() == 0){
        return meta.stream().mapToInt(Integer::intValue).sum();
      }else{
        Integer sum = 0;
        for(Integer index : meta){
          if(index <= children.size()){
            sum += children.get(index -1).weirdSum();
          }
        }
        return sum;
   ...
   * those child nodes. A metadata entry of 1 refers to the first child node, 2 to the second, 3
   ...
   *
   * Node C has one metadata entry, 2. Because node C has only one child node, 2 references a
   * child node which does not exist, and so the value of node C is 0.
   * Node A has three metadata entries: 1, 1, and 2. The 1 references node A's first child node,
   * B, and the 2 references node A's second child node, C. Because node B has a value of 33 and
